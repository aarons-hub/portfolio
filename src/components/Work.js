...
export const Work = () => {
  ...
  const web = data.filter((i) => {
    return i.category === "Web";
  });
  const photography = data.filter((i) => {
    return i.category === "Photography";
  });
  const design = data.filter((i) => {
    return i.category === "Graphics";
  });
    ...
    <>
      <div className="work-container-main">
        ...
        <div className="work-wrapper">
          <h1 className="custom-heading">Webdesign and development</h1>
          <div className="work-container web">
            {web.map((i) => {
              return (
                <div key={i.id} className="item-container">
                  <h2>id: {i.id}</h2>
                  <h2>title: {i.title}</h2>
                  <Link to={`/work/${i.id}`} className="image">
                    <img id={`image-${i.id}`} src={i.image} alt={i.title} />
                  </Link>
                </div>
              );
            })}
        ...
        <div className="work-wrapper">
          <h1 id="photography" className="custom-heading">
            Photography
          </h1>
          <div className="work-container photography">
            {photography.map((i) => {
              return (
                <div key={i.id} className="item-container">
                  <h2>id: {i.id}</h2>
        ...
        <div className="work-wrapper">
          <h1 id="graphic" className="custom-heading">
            Graphic design
          </h1>
          <div className="work-container design">
            {design.map((i) => {
              return (
                <div key={i.id} className="item-container">
                  <h2>id: {i.id}</h2>
